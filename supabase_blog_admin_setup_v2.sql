-- Dr. Rajeev Agarwal website setup v2
-- Use this file instead of the earlier SQL if RPC grant ordering caused errors.
-- Project: etwprdlmxirnuejxxuif
...
drop policy if exists "blogs authenticated can insert" on public.blogs;
drop policy if exists "blogs authenticated can update" on public.blogs;
drop policy if exists "blogs authenticated can delete" on public.blogs;
...
for insert
to authenticated
with check (bucket_id = 'blog-images' and lower(name) like '%.webp');
