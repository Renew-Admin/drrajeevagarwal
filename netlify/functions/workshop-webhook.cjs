...
exports.handler = async (event) => {
  ...
  const webhookUrl = process.env.WORKSHOP_WEBHOOK;

  if (!webhookUrl) {
    return jsonResponse(500, { error: 'WORKSHOP_WEBHOOK is not configured.' });
  }
  ...
  try {
    ...
    if (!webhookResponse.ok) {
      ...
      return jsonResponse(502, {
        error: 'Webhook rejected the workshop payload.',
    ...
    return jsonResponse(502, {
      error: 'Workshop webhook delivery failed.',
