...
/**
 * Canonical absolute URL for an article: https://drrajeevagarwal.co.in/<slug>/
 * Built from the slug only — never from a request path — and mirrors
 * articlePath() in src/utils/blogRoutes.js, the browser-side twin.
 */
function articleUrl(slug) {
  return `${SITE_ORIGIN}/${String(slug || '').replace(/^\/+|\/+$/g, '')}/`;
}
...
function getMetaForPath(pathname) {
  ...
  // Blog post: /<slug>/ at the site root. Real title/description/image come
  // from blogMeta.cjs, generated by scripts/gen-seo-assets.mjs from the post
  // body. The canonical is built from the slug, never from the request path,
  // so /<slug> and /<slug>/ both declare the trailing-slash form. Service
  // pages share this level but are matched by ROUTE_META above, so they win.
  // The old /blog/<slug> form is deliberately not recognised: it 301s to
  // /<slug>/ (src/worker.js) and must never carry article metadata again.
  if (segments.length === 1 && blogMeta[segments[0]]) {
    const post = blogMeta[segments[0]];
    return {
      title: post.title,
      headline: post.headline,
      description: post.description,
      canonicalUrl: articleUrl(segments[0]),
