...
Do not use `npx wrangler deploy` for this project. That command is for Workers and will fail with "Missing entry-point to Worker script or to assets directory". If Cloudflare asks for a custom deploy command, use `npm run deploy:cloudflare` or `npx wrangler pages deploy build --project-name=<your-cloudflare-pages-project-name>`.
...
If deploy fails with "The Pages project does not exist", set `CLOUDFLARE_PAGES_PROJECT_NAME` in Cloudflare project environment variables to the exact project name shown in Workers & Pages. This is the first part of the `.pages.dev` URL. For example, if the URL is `https://my-site.pages.dev`, set `CLOUDFLARE_PAGES_PROJECT_NAME=my-site`.
