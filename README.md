...
The build and deploy scripts run `scripts/prepare-cloudflare-worker-build.mjs` to remove stale Worker-incompatible output before upload. It deletes `build/_redirects` and unused copied WordPress optimizer/plugin backup folders from `build/assets` (`al_opt_content` and `backup`). This keeps the Worker static asset manifest under Cloudflare's 20,000-file limit while leaving the source files in `public/` untouched.
